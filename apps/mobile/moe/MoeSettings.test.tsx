...
const mocks = vi.hoisted(() => ({ setItem: vi.fn(), getItem: vi.fn().mockResolvedValue(null), haptic: vi.fn() }));
vi.mock('@react-native-async-storage/async-storage', () => ({ default: mocks }));
vi.mock('react-native-safe-area-context', () => ({ SafeAreaView: (props: any) => React.createElement('SafeAreaView', props, props.children), useSafeAreaInsets: () => ({ top: 0, bottom: 24, left: 0, right: 0 }) }));
...
vi.mock('./haptics', () => ({ moeHaptic: mocks.haptic }));
...
beforeEach(async () => { mocks.haptic.mockClear(); mocks.setItem.mockReset().mockResolvedValue(undefined); await setMoePreferences({ appearance: 'system', colorSource: 'dynamic', customColor: '#6750A4', glass: 'liquid', motion: 'lively', haptics: 'crisp', celebration: true, nextActionPrompt: true }); });
...
describe('single appearance and motion pages', () => {
  ...
  it('keeps independent selected states for animation and haptics and auditions without changing the selection', async () => {
    await act(async () => { tree = create(<MoeMotionSettings />); });
    await act(async () => { buttons().find((node) => textOf(node) === '夸张')!.props.onPress(); });
    await act(async () => { buttons().find((node) => textOf(node) === '清脆·强')!.props.onPress(); });
    const selected = buttons().filter((node) => node.props.accessibilityRole === 'radio' && node.props.accessibilityState.checked).map(textOf);
    expect(selected).toEqual(['夸张', '清脆·强']);
    expect(getMoePreferences()).toMatchObject({ motion: 'maximal', haptics: 'strong' });
    act(() => { buttons().find((node) => textOf(node) === 'B · 紧双击')!.props.onPress(); });
    expect(mocks.haptic).toHaveBeenLastCalledWith('auditionDouble');
    expect(getMoePreferences()).toMatchObject({ motion: 'maximal', haptics: 'strong' });
  });
  it('preserves both list completion switches in the shared settings rows', async () => {
    await act(async () => { tree = create(<MoeMotionSettings />); });
    const switches = () => tree.root.findAll((node) => String(node.type) === 'Switch');
    await act(async () => { switches().find((node) => node.props.accessibilityLabel === '清单完成庆祝')!.props.onValueChange(false); });
    expect(getMoePreferences()).toMatchObject({ celebration: false, nextActionPrompt: true });
    await act(async () => { switches().find((node) => node.props.accessibilityLabel === '清单下一步提示')!.props.onValueChange(false); });
    expect(getMoePreferences()).toMatchObject({ celebration: false, nextActionPrompt: false });
  });
});
