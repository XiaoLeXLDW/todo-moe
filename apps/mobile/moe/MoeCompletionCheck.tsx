...
export function MoeCompletionCheck({ reveal, emphasis, motion, color, foreground, size = 24 }: {
  ...
  const maximal = !motion.reduced && motion.particleProfile === 'maximal';
  return <Animated.View pointerEvents="none" accessible={false} accessibilityElementsHidden
    ...
    style={[styles.check, { width: size, height: size,
      transform: [{ scale: emphasis.interpolate(maximal ? {
        inputRange: [0, 0.2, 0.36, 0.72, 0.88, 1],
        outputRange: [motion.pressedScale, motion.completedScale, 2.3, 1.75, 1.08, 1],
    ...
    <Animated.View style={{ ...StyleSheet.absoluteFillObject, borderRadius: size * 0.375, backgroundColor: color,
      opacity: reveal.interpolate(maximal
        ? { inputRange: [0, 0.035, 0.15, 1], outputRange: [0, 1, 1, 1] }
    ...
    <Svg width={size} height={size} viewBox="0 0 24 24">
      <AnimatedPath d="M 6.5 12.3 L 10.2 16 L 17.8 8.3" fill="none" stroke={foreground}
        ...
        strokeDashoffset={reveal.interpolate(maximal
          ? { inputRange: [0, 0.04, 0.3, 1], outputRange: [17, 17, 0, 0] }
          : { inputRange: [0, 0.85, 1], outputRange: [17, 0, 0] })} />
    </Svg>
    </Animated.View>
  </Animated.View>;
}
