...
手机仍运行vc7，已测范围是下述指定界面、数据与内屏More路径；最新干净Dev vc12（源码`87cd7d55df676b1f61c1da98906f18e1e82d71ec`）已构建并通过新的完整工程核验、未安装，Diag9未安装。六次渠道prebuild、124项工程测试和实际APK组件/身份/签名/对齐检查通过；vc10/vc11后续发现旧Widget残留，已标为缺陷样本。目录选择器兼容修复已有274文件/2720项移动回归及对应TypeScript证据，尚待受控切屏实测。详见[收尾记录](FOLLOWUP-20260912.md)和[vc12](docs/versions/0.1.0-dev-vc12-2026-09-12.md)。
