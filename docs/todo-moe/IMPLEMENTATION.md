...
**当前基线：** main `a065b165`，PR #1/#2 已合并并保留 main 保护。当前真机测试候选为 Dev vc20，源码 `dd022855`，已正常覆盖安装到 MIX Fold 2；不是正式放行。它已呈现新的玻璃、底栏和面板，实测发现的取消导航、标题下拉与图标问题已修入下一候选源码。Toast 修复与原有数据保护直接继承。保留旧 `feat/todo-moe` 上尚未合入的文档提交，不删除它。
...
| T04 完成体验 | 已实现，待真机验收 | 稳定窗口Host承载有界只读短反馈；同步测量、原业务立即处理；撤销/失败/离页取消，204项相关回归通过；默认不弹自动下一步窗口，保留本地开关 |
...
| T08 质量收尾 | 进行中 | vc20窄屏已验更多保留键盘、保存/顶部安全、草稿确认/保留、空文件夹创建和普通点击；CANCEL手势真实失败已定位并有回归。新性能对照、宽屏和最终候选检查待做 |
...
**下一步：** 把 vc20 的具体修复、细轮廓与 T04 确认反馈合入一个 vc21 候选，检查普通/最后项确认、快撤、下拉取消与拖动取消，然后完成宽屏和短性能对照。参考源码 `local-lan-device-peeker/android/.../ui/glass` 仅作为只读来源，未修改其他工程。
