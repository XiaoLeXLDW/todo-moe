...
**当前基线：** main `a065b165`，PR #1/#2 已合并并保留 main 保护。当前手机 Dev vc25，源码 `28793438`，APK SHA-256 `f067dc4db6439fad0a428c78a4d6ae879b97862b36315a0090d6931b8de7caf7`。完成/快速撤销尾帧、焦点、Modal庆祝均已验，但真实收件箱液态性能未达门槛，不能正式放行。下一候选已改成硬件绘制记录缓存，原生编译通过、实际效果待验。保留旧 `feat/todo-moe` 的文档提交。

| 任务卡 | 状态 | 当前差距与完成依据 |
|---|---|---|
| T00 接管 | 完成 | 继承当前源码、保护和真机证据；统一使用本入口 |
| T01 视觉语言 | 已实现，待真机整合 | 统一色彩、卡片、输入与浮动操作层参数；主路径及设置/详情宽屏上限 |
| T02 玻璃材质 | 动态采样已验，性能调优中 | vc24录到文字、条纹、色块连续经过原生玻璃；滚动停止后5秒1044→1044帧。液态背景采样成本仍高于关闭，下一候选降低背景采样分辨率，保持全分辨率前景/高光 |
| T03 液态底栏 | 外屏交互已验，内屏待验 | vc22普通点击、拖动CANCEL保持原页，以及后续六次0/30/50/80ms点击均通过；单透镜和键盘让位仍须合并内屏观察 |
| T04 完成体验 | 代表性路径已验 | vc24Modal内最后项显示600ms庆祝，Toast仍在上层；绿勾淡出后挂空态。vc25普通完成/撤销及快速撤销已复查，按操作ID同步隐藏旧paint，恢复不再与延迟邻行交错；原业务立即处理、不改core |
| T05 清单组织 | 已实现，待真机验收 | 空文件夹可见/直接新增、文件夹管理、长标题、失败保留输入和返回滚动；72项相关回归通过 |
| T06 新增与详情 | 主路径和焦点修复已验 | 固定KeyboardAvoidingView原生父节点后，vc24下拉→取消的EditText focused=true、mInputShown=true、草稿保留；Save底1478<IME顶1511、标题顶208>状态栏底97。临时诊断正在移除 |
| T07 页面一致与适配 | 内外屏基础路径及大字体已验 | 内屏三页、长标题、QuickCapture安全区、原生日期、保存并编辑/描述/返回、分组新增已验；1.3倍字体主页面/设置未重叠截断，已恢复1.0。系统浮动键盘可覆盖分组编辑器，隐藏键盘后可保存 |
...
**当前短性能对照：** vc24，MIX Fold 2外屏440dpi、机温30.2℃，已有玻璃验证页预热一组后执行三组500ms往返滚动，无录屏。关闭：469帧、P95 13ms、jank 8/1.71%；液态：481帧、P95 18ms、jank 78/16.22%。这是该固定场景的观测，不是所有设备预算；下一候选使用同场景对照。

vc25降低背景采样后压力页P95仍18ms，没有证明帧时间收益；真实收件箱关闭308帧/P95 13ms/jank 0.97%，液态185帧/P95 20ms/jank 39.46%。因此改用单一RenderNode硬件记录主链，待同场景真机检验，不以默认关闭玻璃代替交付。该提交的CI失败为大型存储测试缺少导航host mock导致Flow导入错误；补齐mock后原6用例及预算通过，远端待重跑。

**升级数据对照基线：** vc25热态系统文件夹导出成功；`vc25-hot-backup.json`本地证据27,697字节，设备/电脑SHA-256同为`d7c687356e1da24194ef9a54a1db5336a6170f5b3c6211518d3ec15162983c7a`。含tasks/projects/sections/areas分别32/2/3/2（含本轮自建夹具），下一候选冷启动导出前不修改任务数据。

vc26新硬件记录在进入高级设置时发生RenderThread循环引用闪退；已临时把手机玻璃切为关闭维持操作，未作为交付通过。回归修复以弱引用保留曾含玻璃的祖先，排除系统转场保存的旧子节点，并在读取帧前同步应用排除版本；原生8项检查通过，下一候选待实机确认。vc26关闭玻璃后冷启动导出成功，仍为27,697字节、32/2/3/2；与vc25热备份所有字段及数组顺序的规范化SHA-256同为`a02e81602d8b3d57e1571cb94c2934611f2a0d529a68bbef3c8473b61b7c0717`，数据一致，不代表图形回归已修复。

vc27液态开启下，高级设置往返已正常；相同收件箱滚动液态P95降至16ms、jank12.10%（关闭11ms/约1.96%）。但前台静止仍约309帧/5秒；后台CPU计数5秒不变。vc28只作必要私有诊断，确认可见文字控件长期layoutRequested=true而dirty=false，触发错误源更新。已移除该触发条件与全部探针，依然根据实际dirty、已提交几何/层级变化刷新，下一无诊断候选需确认静止停止与动态更新同时正常。
