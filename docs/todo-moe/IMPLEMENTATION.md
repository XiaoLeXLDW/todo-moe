...
**当前基线：** main `a065b165`，PR #1/#2 已合并并保留 main 保护。当前可安装候选为 Dev vc19，源码 `685d3619`，已正常覆盖安装到 MIX Fold 2。Toast 隐形热区已修复；完成退场已生效，快速撤销的极淡瞬时残影记录为已知视觉限制，不把旧候选重新跑一遍。保留旧 `feat/todo-moe` 上尚未合入的文档提交，不删除它。
...
| T03 液态底栏 | 已实现，待真机验收 | 单透镜、阻尼拖动、取消回位、一次松手导航和键盘连续让位；入口/生命周期16项、玻璃/几何4项回归通过 |
| T04 完成体验 | 已有基础，待成品整合 | 继承 PR #2；补最后项确认、空态交接与可中断反馈，必要只读视觉快照可用 |
| T05 清单组织 | 已实现，待真机验收 | 空文件夹可见/直接新增、文件夹管理、长标题、失败保留输入和返回滚动；72项相关回归通过 |
| T06 新增与详情 | 已实现，待真机验收 | 默认归属和日期、更多保留键盘、玻璃面板进退、标题下拉、草稿保护；93项相关回归通过；关闭后卸载仍有请求身份保护 |
| T07 页面一致与适配 | 待完成 | 主要页面、窄屏/内屏、大字体、基本可访问性；保留系统原生选择器 |
| T08 质量收尾 | 待完成 | 单一候选代表路径短测、新玻璃性能对照及升级数据保护，不重复历史矩阵 |
...
**下一步：** 冻结已整合源码，构建一个统一候选验证 T01–T03/T05–T06 的真实路径；随后补 T04 的确认可见/可取消及 T07–T09。首次整合类型检查已通过，不把原生编译与mock结果当成画面验收。参考源码 `local-lan-device-peeker/android/.../ui/glass` 仅作为只读来源，未修改其他工程。
