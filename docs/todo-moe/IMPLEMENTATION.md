...
**当前基线：** main `a065b165`，PR #1/#2 已合并并保留 main 保护。当前真机测试候选为 Dev vc22，源码 `d0d626ff`，已正常覆盖安装到 MIX Fold 2；不是正式放行。APK SHA-256 `e35d58a1515eae9342e8cef2ac831fe86f027fe552acf68f4f6a2c44418768d6`。vc21 已验证下拉草稿确认、拖动 CANCEL 不导航、普通完成的绿色勾选/划线/淡出及撤销恢复；vc22 修复拖动后首次点击与取消草稿确认后的原生焦点恢复，正在复测。快速撤销仍有原业务两次恢复提交之间的短暂位置/星标中间态，未视为通过。保留旧 `feat/todo-moe` 上尚未合入的文档提交，不删除它。
...
| T04 完成体验 | 普通路径已验，两处交接待修 | vc21普通完成绿色勾选/划线/淡出与撤销恢复可见；vc22清单最后项可见确认，但空态过早重叠；快速撤销会显示status与focus两次恢复之间的中间行。原业务立即处理、不改core |
...
| T08 质量收尾 | 进行中，有真实性能RED | vc22同一Today页面静置5秒：液态6456→6765帧、关闭8130→8130、柔和8868→8868。说明液态仍持续重绘，不能认定空闲预算通过；无录屏对照，后续用最小原生计数定位触发源。宽屏和最终候选检查待做 |
...
**下一步：** 完成 vc22 交互复测，收紧快速撤销中间态的显示，随后完成最后项、宽屏和短性能对照。当前自建验收数据为文件夹 `953020`、清单 `953024`、任务 `953022`；不操作用户既有任务。参考源码 `local-lan-device-peeker/android/.../ui/glass` 仅作为只读来源，未修改其他工程。
