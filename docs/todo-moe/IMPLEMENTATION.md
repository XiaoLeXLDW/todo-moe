...
**当前正式候选（2026-09-13）：** Stable vc33 已使用新正式密钥签名并下载，APK SHA-256 `697d13d4dae339291bb119b9d2efc52903e60162ae5e0b2abec1aa8f5a151ae8`。源码 `c334a17366cb70993e69e7ec9391713c497aa35b`；PR #3/#4/#5 已合并。签名 v2/v3、证书、16 KB ZIP 对齐、包身份及签名前后内容校验通过。按用户要求，本轮停止手机操作，尚未安装或公开发布。详见 [Stable vc33 签名记录](docs/versions/0.1.0-stable-vc33-signed-2026-09-13.md)。以下 Dev 记录为既有验收基线。

**交付目标：** 清爽内容层、精致玻璃操作层、克制的家族细节；完成整套日常路径后交付正式候选。玻璃与底栏允许重做、适配 SukiSU；不再执行旧的“冻结玻璃、先发布再美化”优先级。

**Dev 验收基线：** Dev vc32 已覆盖安装（干净源码 `9524df7cf7deb9e16d8fdc4b5701dc10cc8afd63`，APK SHA-256 `bf2da8b13c2fcd47c35bc3519a657edcb83d41d2544556cbbe33aff3b05df7bf`）。原版Compose底栏已正常冷启动，resource-id `moe-sukisu-navigation` 确认新路径；来回拖动、透镜外扩、镜内图标文字放大/着色/折射及新增面板隐藏/返回已观察。用户已否决vc30自绘底栏；用户于2026-09-13确认vc32观感无问题；新底栏内屏检查已通过，正式签名发布仍待完成。PR #1/#2 已合并，main保护保留。

**底栏重做进度：** 完整SukiSU Compose组件经原生View桥接RN页面背景；组合透镜、着色标签副本、弹簧和双光源保留。原生编译与9项测试、JS导航桥接22项测试及TypeScript检查通过。vc31首启失败由Fabric挂载前测量触发；vc32将Compose子视图测量限制在挂载后，并修正模式切换时高光引用旧动画，真机首启通过。旧底栏证据不能替代新组件的完整验收。

| 任务卡 | 状态 | 当前差距与完成依据 |
|---|---|---|
| T00 接管 | 完成 | 继承当前源码、保护和真机证据；统一使用本入口 |
| T01 视觉语言 | 已实现，代表路径已验 | 统一色彩、卡片、输入与浮动操作层参数；内外屏基础布局和宽屏上限已有实测，vc30底栏曾被否决，vc32原版移植已获用户观感确认 |
| T02 玻璃材质 | 原版移植及真机初验完成 | vc32实际使用完整Compose原版组件，镜内着色副本/放大/折射和背景进入底栏已观察，用户已确认观感；旧vc29性能数字不代表新renderer |
| T03 液态底栏 | 新组件代表路径已验 | vc32上沿拖动今天→收件箱→今天、中心反向拖动已成功；按住透镜外扩；新增面板打开时隐藏且关闭后恢复。vc32已实测透镜拖至收件箱后CANCEL回到今天；2026-09-13内屏深色布局、拖动至收件箱、点击清单、新增面板隐藏及返回今天均通过 |
...
| T08 质量收尾 | 新组件代表性收尾进行中 | vc32静止5秒2542→2542帧，后台3秒CPU计数不变，同进程恢复新底栏；ec86f142三组CI全部成功。2026-09-13内屏代表路径通过，不能外推全部设备 |
| T09 正式交付 | 正式签名及电脑端交付已完成 | Stable vc33 已签名并下载，独立校验通过；约34秒的 Dev vc32 沙盒演示已准备。手机安装/迁移/验收按用户安排延后，公开发布未批准 |
...
**旧GlassSurface性能记录（不是vc32 Compose底栏预算）：** vc29真实收件箱同场景，关闭316帧、P95 13ms、jank 2.22%；液态321帧、P95 18ms、jank 19%。P95为直方图桶近似，限定于本机该次场景，液态相对关闭仍有额外成本；当前未定义要求两者相等的数值门槛。静止5秒71→71帧，后台CPU计数3秒不变，只证明相应观测区间停止无效工作。vc26的RenderThread循环引用闪退及vc27静止持续重绘保留为失败记录；后续已修复祖先排除和错误layoutRequested触发条件，vc29为移除临时探针后的验证结果，不代替长期资源或所有场景验收。
...
**下一步：** 不需要手机的构建、正式签名、文件校验和交付文档已完成。用户当前要使用手机，停止所有手机操作；待其方便后再安装 Stable vc33 并进行基础验收及所需数据迁移。正式发布未执行，受保护 release 阶段保持等待。文档更新保存在交付文档分支，待验收后统一合并。
