# Todo Moe 产品整合：当前执行入口

当前目标以 [2026-09-13 产品整合任务](PRODUCT-INTEGRATION-20260913.md) 为准。基于已公开的 0.1.0 / Stable vc33，开发 0.2.0 候选；最新要求覆盖旧版已接受的主题和动画观感。唯一设置首页、系统动态色/自定义颜色、增强动画与预览、产品 About/帮助/隐私、随包许可已进入实现；图标已按用户授权提取真透明前景；0.2.0 Dev vc35 已构建并集中验收，用户确认可以作为候选。正式0.2.0包与发布仍待后续步骤。

正式版按用户选择从空白开始，Dev 不迁移，同步不启用。vc33 已公开发布且补充原始源码 AGPL 全文附件，原 APK/四项签名材料不变。后继版本沿用正式身份与密钥，不覆盖旧 APK。

---

以下为此前 0.1.0 冲刺及验收历史；不代表本轮新目标已经完成。
