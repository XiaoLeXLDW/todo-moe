...
最新候选为 [0.3.0 Dev vc49](0.3.0-dev-vc49-2026-09-16.md)，源码 `95b61df5`。vc49 修复项目实际归档后无返回、过期元数据回调先触发阻塞和三种主语言快速上手仍显示 Sam；历史 Sam 示例保留为只读兼容识别，用户任务不被改写。UTC Core 3876 pass / 8 skip、Mobile 296 files / 2923 tests 通过；干净 APK 构建和 USB `zizhan` 覆盖安装通过，归档→重新激活闭环恢复 5 项清单状态。[Dev vc12](0.1.0-dev-vc12-2026-09-12.md)及更早记录是历史快照，不再代表当前候选。
...
**更新：2026-09-16。目标手机当前安装 Dev vc49；最新已构建 Dev 也是 vc49；v1.0 未放行，本轮不发布 Release。** 早期 vc12 与 `5a36c0b1b7b4091cbff6c839676dff1fa26e6a8a` 的 CI 结果仅作历史证据，不沿用到本轮源码。见[详细回读](../../CI-VALIDATION-20260911.md)。用户明确本轮 local-only，不要求为验收启用任务同步。
