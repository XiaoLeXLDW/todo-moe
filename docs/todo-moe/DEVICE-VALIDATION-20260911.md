...
当前设备仍为vc7；干净vc10和诊断vc9已经准备、尚未安装。用户已确认分享回退发生前没有点击返回或取消。后续生产兼容修复与切屏复测条件见[收尾记录](FOLLOWUP-20260912.md)，不能用已通过的构建或单测代替该次设备复测。
...
首次展开后的热路径导出转入Android系统分享，未发送；用户已确认未取消前面的选择器。同内屏冷重启后的SAF导出成功，vc7首次导出也成功。只读审查未发现本轮生产export或Activity配置差异，当前仅登记原因未定的观察，不能认定fold/Expo缺陷已证实，或被vc7修复。
