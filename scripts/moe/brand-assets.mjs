...
const sourcePath = fileURLToPath(new URL('family-mascot-v3-skin.png', target));
...
const files = ['family-mascot-v1-source.png','family-mascot-v2-transparent.png','family-mascot-v3-skin.png','icon.png','foreground.png','background.png','monochrome.png','monochrome.svg'];
writeFileSync(fileURLToPath(new URL('asset-manifest.json', target)), JSON.stringify({ artwork: 'family-mascot-checklist-v3-skin', generator: 'built-in image_gen skin edit + face-only extraction preserving original alpha', packaging: 'Node 22 / pinned Expo image-utils Jimp backend', files: files.map(file => { const b = readFileSync(fileURLToPath(new URL(file,target))); return { file, bytes:b.length, sha256:createHash('sha256').update(b).digest('hex') }; }) }, null, 2)+'\n');
